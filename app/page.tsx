...
export default function Home() {
  ...
  const history = useMemo(() => {
    ...
    try {
      return JSON.parse(localStorage.getItem('steamsearch-history') ?? '[]') as string[];
    } catch {
      return [];
    }
  }, [result]);
  ...
  const saveHistory = (value: string) => {
    ...
    const current = history.filter((item) => item !== trimmed);
  ...
  return (
    <main>
      ...
      <section className="search-panel">
        ...
        <div className="hints">
          Например: "{suggestions[Math.floor(Math.random() * suggestions.length)]}"
        </div>
      </section>
      ...
      {history.length > 0 && (
        <section className="history">
          ...
          <div className="actions" style={{ marginTop: '0.6rem' }}>
            {history.map((item) => (
              <button key={item} className="secondary" onClick={() => runSearch(item)}>
                {item}
              </button>
            ))}
          </div>
          ...
          <div className="card-grid">
            <article className="card">
              ...
              <div className="card-body">
                ...
                <div className="actions" style={{ marginTop: '0.6rem' }}>
                  <a href={result.results[0].link} target="_blank" rel="noreferrer">
                    <button className="primary">Открыть в Steam</button>
                  </a>
                  <button className="secondary" onClick={() => copy(result.results[0].link)}>
                    Скопировать ссылку
                  </button>
                </div>
              </div>
              ...
              <article className="card" key={game.appid}>
                ...
                <div className="card-body">
                  ...
                  <div className="actions" style={{ marginTop: '0.6rem' }}>
                    ...
                    <button className="secondary" onClick={() => copy(game.link)}>
                      Скопировать
                    </button>
                  </div>
                </div>
